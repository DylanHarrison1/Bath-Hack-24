function createNumberPlate(seed) {
    ...
    let plate = areaCodes[Math.floor(Math.random() * areaCodes.length)].replace(" ", "");
    plate += dates[Math.floor(Math.random() * dates.length)];
    plate += " ";
    plate += alphabet[Math.floor(Math.random() * alphabet.length)] + alphabet[Math.floor(Math.random() * alphabet.length)] + alphabet[Math.floor(Math.random() * alphabet.length)];
    return plate;
}
